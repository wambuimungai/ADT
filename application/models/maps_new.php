<?php
class Maps_New extends Doctrine_Record {
	...
	public function setUp() {
		$this -> setTableName('maps_new');
		$this -> hasOne('New_Sync_Facility as S_Facility', array('local' => 'facility_id', 'foreign' => 'id'));
	}

	public function getAll() {
		$query = Doctrine_Query::create() -> select("*") -> from("maps_new");
		$maps = $query -> execute();
		return $maps;
	}

	public function getMap($id){
		$query = Doctrine_Query::create() -> select("*") -> from("maps_new")->where("id =? ",array($id));
		$maps = $query -> execute(array(), Doctrine::HYDRATE_ARRAY);
		return $maps;
	}

	public function getPeriods() {
		$query = Doctrine_Query::create() -> select("Distinct(period_begin) as periods") -> from("maps_new")->orderBy("period_begin desc");
		$maps = $query -> execute();
		return $maps;
	}
