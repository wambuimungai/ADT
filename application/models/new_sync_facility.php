<?php
class New_Sync_Facility extends Doctrine_Record {
    ...
    public function setUp() {
        $this -> setTableName('new_sync_facility');
    }

    public function getAll() {
        $query = Doctrine_Query::create() -> select("*") -> from("new_sync_facility");
    ...
    public function getId($facility_code, $status_code = 0) {
        ...
        $query = Doctrine_Query::create() -> select("id") -> from("new_sync_facility") -> where("$conditions");
        $sync_facility = $query -> execute(array(), Doctrine::HYDRATE_ARRAY);
    ...
    public function getCode($facility_id, $status_code = 0) {
        ...
        $query = Doctrine_Query::create() -> select("code") -> from("new_sync_facility") -> where("$conditions");
        $sync_facility = $query -> execute(array(), Doctrine::HYDRATE_ARRAY);
        return @$sync_facility[0];
    }

    public function getSatellites($central_site) {//Include CUrrent facility
        $query = Doctrine_Query::create() -> select("id") -> from("new_sync_facility") -> where("parent_id='$central_site'");
        $sync_facility = $query -> execute(array(), Doctrine::HYDRATE_ARRAY);
        return $sync_facility;
    }

    public function getOtherSatellites($central_site, $facility_code) { //Only get satellites
        $query = Doctrine_Query::create() -> select("id") -> from("new_sync_facility") -> where("parent_id='$central_site' and code !='$facility_code'");
        $sync_facility = $query -> execute(array(), Doctrine::HYDRATE_ARRAY);
        return $sync_facility;
    }

    public function getSatellitesDetails($central_site) {
        $query = Doctrine_Query::create() -> select("*") -> from("new_sync_facility") -> where("parent_id='$central_site'");
        $sync_facility = $query -> execute(array(), Doctrine::HYDRATE_ARRAY);
        return $sync_facility;
    }
    ...
    {
        $query = Doctrine_Query::create() -> select("category") -> from("new_sync_facility") -> where("code='$code'");
        $sync_facility = $query -> execute(array(), Doctrine::HYDRATE_ARRAY);
