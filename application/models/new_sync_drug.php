<?php
class New_Sync_Drug extends Doctrine_Record {
    ...
    public function setUp() {
        $this -> setTableName('new_sync_drug');
    }

    public function getAll() {
        $query = Doctrine_Query::create() -> select("*") -> from("new_sync_drug");
    ...
    public function getActive() {
        ...
        $query = Doctrine_Query::create() -> select("id,packsize,$drug_name") -> from("new_sync_drug") -> where("category_id='1' or category_id='2' or category_id='3'") -> orderBy("category_id asc");
        $sync_drug = $query -> execute(array(), Doctrine::HYDRATE_ARRAY);
        return $sync_drug;
    }
    ...
    public function getActiveList() {
        ...
        $query = Doctrine_Query::create() -> select("id,$drug_name") -> from("new_sync_drug") -> where("category_id='1' or category_id='2' or category_id='3'") -> orderBy("category_id asc");
        $sync_drug = $query -> execute();
        return $sync_drug;
    }

    public function getPackSize($id) {
        $query = Doctrine_Query::create() -> select("packsize") -> from("new_sync_drug") -> where("id='$id'");
        $sync_drug = $query -> execute(array(), Doctrine::HYDRATE_ARRAY);
        return $sync_drug[0];
    }
