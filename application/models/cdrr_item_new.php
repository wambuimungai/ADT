<?php
class Cdrr_Item_New extends Doctrine_Record {
	...
	public static function getOrderItems($cdrr) {
		$query = Doctrine_Query::create() -> select("*") -> from("cdrr_item_new") -> where("cdrr_id = '$cdrr'");
		$items = $query -> execute();
		return $items;
	}

	public function getLastPhysicalStock($period_begin, $drug_id, $facility_id) {
		$query = Doctrine_Query::create() -> select("count") -> from("cdrr_item_new ci") -> where("ci.Cdrr.period_begin = '$period_begin' and ci.drug_id='$drug_id' and ci.Cdrr.facility_id='$facility_id' and ci.Cdrr.status !='prepared' and ci.Cdrr.status !='deleted'") -> OrderBy("ci.Cdrr.id desc");
                $items = $query -> execute();
